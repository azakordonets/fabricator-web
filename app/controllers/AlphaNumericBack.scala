...
object AlphaNumericBack extends Controller {
  ...
  private def generateInt(min: Int, max: Int): Int = (min, max) match {
    ...
    case (0, 0) => alpha.randomInt
    case (`min`, `max`) if min > 0 && max == 0 => alpha.randomInt(min, 1000000000)
    case (`min`, `max`) if max > 0 && min == 0 => alpha.randomInt(0, max)
    case (`min`, `max`) if min > 0 && max > 0 => alpha.randomInt(min, max)
  }
  ...
  private def generateDouble(min: Double, max: Double, accuracy: Int): Double = (min, max, accuracy) match {
    ...
    case (0.0, 0.0, `accuracy`) if accuracy < 1 => alpha.randomDouble
    case (0.0, 0.0, `accuracy`) if accuracy >= 1 => BigDecimal(alpha.randomDouble).setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble

    case (`min`, `max`, `accuracy`) if min > 0 && max == 0 && accuracy == 1 => alpha.randomDouble(min, 1000000000)
    case (`min`, `max`, `accuracy`) if min > 0 && max == 0 && accuracy >= 1 => BigDecimal(alpha.randomDouble(min, 1000000000)).
      setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble

    case (`min`, `max`, `accuracy`) if max > 0 && min == 0 && accuracy == 1 => alpha.randomDouble(0, max)
    case (`min`, `max`, `accuracy`) if max > 0 && min == 0 && accuracy >= 1 => BigDecimal(alpha.randomDouble(0, max)).
      setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble

    case (`min`, `max`, `accuracy`) if min > 0 && max > 0 && accuracy == 1 => alpha.randomDouble(min, max)
    case (`min`, `max`, `accuracy`) if min > 0 && max > 0 && accuracy >= 1 => BigDecimal(alpha.randomDouble(min, max)).
      setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble
  }
  ...
  def integerRange(min: Int, max: Int, step: Int, json: Boolean) = Action {
    try {
      if (step < 1) BadRequest("Step should be more then 1")
        ...
        val generatedResult = alpha.randomIntsRange(min, max, step)
        if (json) Ok(Json.toJson(integerSeq(generatedResult))) else Ok(Json.toJson(generatedResult))
      }
    } catch {
      case e: IllegalArgumentException => BadRequest(e.getMessage)
  ...
  def doubleRange(min: Double, max: Double, step: Double, json: Boolean) = Action {
    try {
      if (step < 1) BadRequest("Step should be more then 1")
        ...
        val generatedResult = alpha.randomDoublesRange(min, max, step)
        if (json) Ok(Json.toJson(doubleSeq(generatedResult))) else Ok(Json.toJson(generatedResult))
      }
    } catch {
      case e: IllegalArgumentException => BadRequest(e.getMessage)
  ...
  def string(length: Int, json: Boolean, amount: Int) = Action {
    if (amount < 1) BadRequest("Amount should be more then 1")
      ...
      val result = if (length == 30) alpha.randomString else alpha.randomString(length)
      if (json) Ok(Json.toJson(Map("string" -> result))) else Ok(Json.toJson(result))
    } else {
      val result = Seq.fill(amount)(if (length == 60) alpha.randomString else alpha.randomString(length))
      if (json) Ok(Json.toJson(Map("string"->result))) else Ok(Json.toJson(result))
    }
  }
  ...
  def strings(min: Int, max: Int, amount: Int, json: Boolean) = Action {
    try {
      if (amount < 1) BadRequest("Amount should be more then 1")
        ...
        val generatedResult = alpha.randomStrings(min, max, amount)
  ...
  def hash(length: Int, json: Boolean, amount: Int) = Action {
    if (amount < 1) BadRequest("Amount should be more then 1")
      ...
      val result = if (length == 40) alpha.randomHash else alpha.randomHash(length)
      if (json) Ok(Json.toJson(Map("randomHash" -> result))) else Ok(Json.toJson(result))
    } else {
      val result = Seq.fill(amount)(if (length == 40) alpha.randomHash else alpha.randomHash(length))
  ...
  def hashesList(min: Int, max: Int, amount: Int, json: Boolean) = Action {
    try {
      if (amount < 1) BadRequest("Amount should be more then 1")
        ...
        val generatedResult = alpha.randomHashList(min, max, amount)
  ...
  def guid(length: Int, json: Boolean, amount: Int) = Action {
    if (amount < 1) BadRequest("Amount should be more then 1")
      ...
      val result = if (length == 5) alpha.randomGuid else alpha.randomGuid(length)
      if (json) Ok(Json.toJson(Map("randomGuid" -> result))) else Ok(Json.toJson(result))
    } else {
      val result = Seq.fill(amount)(if (length == 5) alpha.randomGuid else alpha.randomGuid(length))
